    ...
    return [
      {
        type: 'keyword',
        regex:
          '\\b(function|return|const|let|var|if|else|for|while|class|import|export|async|await|try|catch|finally)\\b',
        priority: 10,
      },
      {
        type: 'string',
  ...
  getPatterns() {
    return [
      ...
      },
      {
        type: 'keyword',
        regex:
          '\\b(def|class|return|if|else|elif|for|while|import|from|as|try|except|finally|with|yield|lambda|pass|break|continue|and|or|not|in|is)\\b',
        priority: 9,
      },
      {
        type: 'builtin',
  ...
  getPatterns() {
    return [
      ...
      },
      {
        type: 'keyword',
        regex:
          '\\b(class|interface|extends|implements|return|if|else|for|while|new|this|super|try|catch|finally|throw|throws|import|package)\\b',
        priority: 8,
      },
      {
        type: 'type',
        regex:
          '\\b(String|int|Integer|boolean|Boolean|double|Double|float|Float|char|Character|long|Long|short|Short|byte|Byte|void)\\b',
        priority: 7,
      },
      {
        type: 'string',
  ...
  getPatterns() {
    return [
      ...
      },
      {
        type: 'keyword',
        regex:
          '\\b(struct|class|enum|protocol|func|var|let|return|if|else|guard|for|while|switch|case|default|import|extension|init|deinit|override|final|static|private|public|internal|fileprivate|open)\\b',
        priority: 9,
      },
      {
        type: 'type',
...
class VisualEffectsProcessor {
  ...
  splitTokenForSelection(token, selectionStart, selectionEnd) {
    ...
    if (token.start < selectionStart) {
      ...
      tokens.push(new Token(token.type, value.substring(0, beforeLength), token.start, selectionStart))
    }
    ...
    if (token.end > selectionEnd) {
      ...
      tokens.push(new Token(token.type, value.substring(afterStartInToken), selectionEnd, token.end))
    }
...
class NeovimHandler {
  ...
  processCode(sourceCode, language, mode, selectionStart, selectionEnd) {
    ...
    const processedTokens = this.visualEffectsProcessor.process(tokens, mode, selectionStart, selectionEnd)

    // Render the tokens
    return renderer.render(processedTokens)
  }
